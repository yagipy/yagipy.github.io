...
特にGo/Node.jsを使用したAPIサーバーやライブラリの開発、AWSを用いたインフラ/DevOps構築を得意としています。  
...
多くのユーザーが存在しているサービスは、ユーザーが少ないサービスより社会的な存在意義が高いことが多いと考えているのと、多くのユーザーが存在している際に起きる技術的な問題は情報が少なく、よりチャレンジングな問題に取り組める可能性が高いと考えています。
...
- KPIの立案・作成・運用
- コンピテンシーマトリクス(エンジニア評価制度)の立案・作成・運用
...
- Webフロントエンド、バックエンドのプロジェクトマネジメント(メンバー6名)
  - スケジュール、進捗管理
  - 品質管理(レビュー、ペアプロ)
- 業務委託面談
...
#### 担当業務 
主にReactを使用してWebフロントエンドの実装を行いました。  
react-pdfを使用したWebフロントでのPDF生成、複数ファイルのアップロード機能、react-tableを使用した週次カレンダー機能等を実装しました。  
APIサーバーについてもいくつかAPIを実装しており、各区分ごとにソートを行う処理やパフォーマンスの最適化等を実装しました。
...
1人目のエンジニア、CTOとして参画。  
大学のサークルを経由して連絡できるマッチングアプリ、「Camel」の構築を担当。
